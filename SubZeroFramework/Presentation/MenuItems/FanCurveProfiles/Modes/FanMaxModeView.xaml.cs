...
/// <summary>Max mode body, resolved by the mode navigation sub-region. DataContext is the <see cref="FanMaxModeModel"/>.</summary>
public sealed partial class FanMaxModeView : UserControl, INotifyPropertyChanged
{
    public FanMaxModeView()
    {
        this.InitializeComponent();
        DataContextChanged += (_, args) =>
        {
            if (args.NewValue is FanMaxModeModel model)
            {
                ViewModel = model;
    ...
    public FanMaxModeModel ViewModel
    {
        get => field;
        set
        {
