...
/// <summary>
/// Storage category body: an instance picker on the left and a detail sub-region (<c>DetailHost</c>) resolved by
...
/// </summary>
public sealed partial class DeviceCapabilitiesStorageCategoryView : UserControl, INotifyPropertyChanged
{
    ...
    public DeviceCapabilitiesStorageCategoryView()
    {
        this.InitializeComponent();
        DataContextChanged += (_, args) =>
        {
            if (args.NewValue is DeviceCapabilitiesStorageCategoryModel model)
            {
                ViewModel = model;
                HookItems();
                EnsureSelection();
    ...
    public DeviceCapabilitiesStorageCategoryModel ViewModel
    {
        get => field;
        set
        {
    ...
    private void HookItems()
    {
        ...
        _itemsHooked = true;
        ((INotifyCollectionChanged)ViewModel.Page.StorageDriveCards).CollectionChanged += (_, _) => EnsureSelection();
    }
    ...
    private void EnsureSelection()
    {
        ...
        if (Picker.SelectedItem is null && ViewModel.Page.StorageDriveCards.Count > 0)
        {
    ...
    {
        if (Picker.SelectedItem is not DeviceCapabilitiesStorageDriveCardModel item || ReferenceEquals(item, _lastNavigatedItem))
        {
            return;
        }
