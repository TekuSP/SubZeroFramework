...
/// <summary>
/// Network category body: an instance picker on the left and a detail sub-region (<c>DetailHost</c>) resolved by
...
/// </summary>
public sealed partial class DeviceCapabilitiesNetworkCategoryView : UserControl, INotifyPropertyChanged
{
    ...
    public DeviceCapabilitiesNetworkCategoryView()
    {
        this.InitializeComponent();
        DataContextChanged += (_, args) =>
        {
            if (args.NewValue is DeviceCapabilitiesNetworkCategoryModel model)
            {
                ViewModel = model;
                HookItems();
                EnsureSelection();
    ...
    public DeviceCapabilitiesNetworkCategoryModel ViewModel
    {
        get => field;
        set
        {
    ...
    private void HookItems()
    {
        ...
        _itemsHooked = true;
        ((INotifyCollectionChanged)ViewModel.Page.NetworkAdapterCards).CollectionChanged += (_, _) => EnsureSelection();
    }
    ...
    private void EnsureSelection()
    {
        ...
        if (Picker.SelectedItem is null && ViewModel.Page.NetworkAdapterCards.Count > 0)
        {
    ...
    {
        if (Picker.SelectedItem is not DeviceCapabilitiesNetworkAdapterCardModel item || ReferenceEquals(item, _lastNavigatedItem))
        {
            return;
        }
