...
/// <summary>
/// Memory category body: an instance picker on the left and a detail sub-region (<c>DetailHost</c>) resolved by
...
/// </summary>
public sealed partial class DeviceCapabilitiesMemoryCategoryView : UserControl, INotifyPropertyChanged
{
    ...
    public DeviceCapabilitiesMemoryCategoryView()
    {
        this.InitializeComponent();
        DataContextChanged += (_, args) =>
        {
            if (args.NewValue is DeviceCapabilitiesMemoryCategoryModel model)
            {
                ViewModel = model;
                HookItems();
                EnsureSelection();
    ...
    public DeviceCapabilitiesMemoryCategoryModel ViewModel
    {
        get => field;
        set
        {
    ...
    private void HookItems()
    {
        ...
        _itemsHooked = true;
        ((INotifyCollectionChanged)ViewModel.Page.MemoryModuleCards).CollectionChanged += (_, _) => EnsureSelection();
    }
    ...
    private void EnsureSelection()
    {
        ...
        if (Picker.SelectedItem is null && ViewModel.Page.MemoryModuleCards.Count > 0)
        {
    ...
    {
        if (Picker.SelectedItem is not DeviceCapabilitiesMemoryModuleCardModel item || ReferenceEquals(item, _lastNavigatedItem))
        {
            return;
        }
