...
public sealed class FrameworkDataProvider : IFrameworkDataProvider, IDisposable
{
	...
	private static readonly ImmutableArray<FrameworkEcDriver> KnownDrivers = Enum
		.GetValues<FrameworkEcDriver>()
		.Where(driver => driver != FrameworkEcDriver.Unknown)
		.ToImmutableArray();
	...
	private FrameworkSystemStatus ReadSystemStatus()
	{
		...
		return new FrameworkSystemStatus
		{
			...
			IsEcPollingEnabled = isLibraryAvailable && isFrameworkDevice == true,
