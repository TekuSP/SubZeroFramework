...
- ✅ Promote display-unit preferences to a machine-wide service-owned surface (`FrameworkUserPreferencesService` + `IUserUnitPreferencesClient`) and reshape Settings around shared Apply/Save/Load (plus Load/Reset Draft) flows for both runtime configuration and display preferences. Custom service-approved settings paths remain deferred.
- ⏳ Allow operators to relocate the service-owned configuration overlay (`service-settings.json`) and machine-wide user preferences (`user-preferences.json`) to a custom directory. Must reuse a single shared service-side "relocate store" flow (validate target writable by LocalSystem on Windows and the service account on Linux, reject UNC/removable/non-existent paths, atomically copy → fsync → swap pointer → delete old → broadcast new path), persist the chosen path in a tiny bootstrap pointer file under the default `ProgramData` location so the service can find it on next start, and expose a client-side Uno `FolderPicker` (see https://platform.uno/docs/articles/features/windows-storage-pickers.html) in Settings that hands the chosen directory to the service over gRPC. Both Advanced service configuration and Units cards should consume the same relocation surface; do not implement two parallel flows.
...
- ✅ Add broad display-unit preferences across the UI for temperature, fan speed, clock frequency, refresh rate, information size, voltage, current, battery charge capacity, ratio or fraction, length, airflow, network link speed, and power. Settings exposes a Units section with Load, Reset Draft, Restore Defaults, Apply, and Save flows backed by the service-owned `FrameworkUserPreferencesService`, so every local client on the machine shares the same units while service and gRPC payloads remain in canonical units.
